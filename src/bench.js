...
const { REDIS_URL, DATA_FILE } = require('./config');
const { sanitize, redisSearchArgs, redisLeiKey, oracleSearchSql, oracleLeiSql } = require('./queries');
...
// query would mostly measure Oracle's filterCache; rotating a realistic spread
...
if (!isMainThread) {
  ...
  (async () => {
    ...
    while (nowMs() < deadline) {
      ...
      try {
        if (engine === 'redis') {
          ...
          await oracle.query(oracleLeiSql().sql, { lei });
        } else {
          const { sql, binds } = oracleSearchSql(
            scenario, sanitize(term), filters, 10, 'relevance'
          );
          await oracle.query(sql, binds);
        }
    ...
    if (engine === 'oracle') await oracle.close();
...
async function main() {
  ...
  for (const engine of ['redis', 'oracle']) {
  ...
  console.log(`\n  throughput ratio: ${(r.qps / s.qps).toFixed(2)}x  (redis / oracle)`);
