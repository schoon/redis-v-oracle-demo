...
// help from Redis or Oracle, and then compared against what the API returns.
//
...
async function main() {
  ...
  record('corpus', 'Oracle doc count == file lines',
    stats.oracle.docs === recs.length, `${stats.oracle.docs.toLocaleString()} vs ${recs.length.toLocaleString()}`);
  ...
  for (const term of ['kes', 'stonebr', 'aber']) {
    ...
    record('prefix', `"${term}*" total == brute force`,
      d.redis.total === expected && d.oracle.total === expected,
      `expected ${expected.toLocaleString()} · redis ${d.redis.total.toLocaleString()} · oracle ${d.oracle.total.toLocaleString()}`);
  }
  ...
  for (const q of ['kestral capitol', 'invermer finacial']) {
    ...
    record('fuzzy', `"${q}" total == brute force`,
      d.redis.total === expected && d.oracle.total === expected,
      `expected ${expected} · redis ${d.redis.total} · oracle ${d.oracle.total}`);
  ...
  // ---- filtered ----------------------------------------------------------
  {
    ...
    record('filtered', 'total == brute force',
      d.redis.total === expected.length && d.oracle.total === expected.length,
      `expected ${expected.length} · redis ${d.redis.total} · oracle ${d.oracle.total}`);
  ...
  for (const [centre, lat, lon, km] of [
    ...
    const ok = Math.abs(d.redis.total - expected) <= tol && Math.abs(d.oracle.total - expected) <= tol;
    record('geo', `${centre} ${km}km == haversine`, ok,
      `expected ${expected} · redis ${d.redis.total} · oracle ${d.oracle.total} · tol ±${tol.toFixed(0)}`);
  }
  ...
  for (const field of ['credit_rating', 'country', 'sector']) {
    ...
    record('breakdown', `${field} counts+sums == brute force`, bad === 0,
      `${d.redis.buckets.length} buckets, ${bad} wrong · agree with Oracle: ${d.bucketsAgree}`);
  }
  ...
  // ---- exact LEI ---------------------------------------------------------
  {
    ...
    for (const i of [0, 1234, 50000, 99999]) {
      ...
      if (!d.redis.found || d.redis.name !== r.legal_name
        || !d.oracle.found || d.oracle.name !== r.legal_name) bad += 1;
    }
    ...
    record('lei', 'unknown identifier not found',
      miss.redis.found === false && miss.oracle.found === false,
      `redis found=${miss.redis.found} · oracle found=${miss.oracle.found}`);
  }
  ...
  if (vectorsAvailable()) {
    ...
    for (const q of questions) {
      ...
      const sRecall = d.oracle.hits.filter((h) => exact.has(h.id)).length;
      ...
      record('semantic', `recall@10 vs exact KNN`, ok,
        `"${q.slice(0, 34)}…" redis ${rRecall}/10 · oracle ${sRecall}/10`);
    }
  ...
  for (const [label, path] of [
    ...
    record('timing', `${label} median is stable`, ok,
      `variation across 7 samples: redis ${rCv.toFixed(0)}% · oracle ${sCv.toFixed(0)}%`);
  }
